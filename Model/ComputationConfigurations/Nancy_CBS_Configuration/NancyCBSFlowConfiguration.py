...
class NancyCBSFlowConfiguration(DefaultFlowConfiguration):

    def __init__(self, avb_class: str = "A", is_periodic: bool = True, is_worst_case: bool = False,
                 max_frame_size: int = 64, class_measurement_interval: Fraction = Fraction(1, 100),
                 max_interval_frame: int = 10, is_plot_in_browser: bool = False, **kwargs):
        ...
        self.class_measurement_interval: Fraction = class_measurement_interval
    ...
    def to_serializable_dict(self) -> dict:
        own_dict = {"avb_class": self.avb_class, "is_periodic": self.is_periodic, "is_worst_case": self.is_worst_case,
                    ...
                    "class_measurement_interval_numerator": self.class_measurement_interval.numerator,
                    "class_measurement_interval_denominator": self.class_measurement_interval.denominator,
    ...
    def to_constructor_dict(json_dict, model, flow):
        ...
        own_dict = {"avb_class": json_dict["avb_class"], "is_periodic": json_dict["is_periodic"],
                    ...
                    "class_measurement_interval": Fraction(json_dict["class_measurement_interval_numerator"],
                                                           json_dict["class_measurement_interval_denominator"]),
                    "max_interval_frame": json_dict["max_interval_frame"]}
        return super_dict | own_dict

    def make_parameter_dict(self):
        own_dict = {"plot in web browser": self.is_plot_in_browser, "AVB class (A/B/NSR)": self.avb_class,
                    "is periodic": self.is_periodic,
                    "is worst case": self.is_worst_case, "max. frame size (Byte)": self.max_frame_size,
                    "class measurement interval (s)": self.class_measurement_interval.__float__(),
    ...
    def update_parameter_dict(self, dictionary):
        ...
        new_value = Fraction(dictionary["class measurement interval (s)"])
        if new_value < 0:
            raise ValueError("Negative Value")
        ...
        self.preview_source_ac()

    def preview_source_ac(self):
        """
        Creates a plot of the associated arrival curve.
        """
        ...
        if self.is_plot_in_browser or nw_config.plot_on_change_in_browser:
            ...
            nw_config.plot_flow_source_ac(self.associated_component)
